...
def compute_loss(pred, true, task, use_focal_loss=False):
    ...
    if task == 'classification':
        ...
        else: #二分类/多标签（pred.ndim == 1 或 true.ndim > 1）
            true = true.float()
            # print("pred.shape =", pred.shape) pred.shape = torch.Size([32])
            # print("true.shape =", true.shape) true.shape = torch.Size([32, 64])
            # assert 0
...
def eval_epoch(model, loader, device, split='val', task='classification', use_focal_loss=False):
    ...
    for batch in tqdm(loader, desc=f"eval_{split}", leave=False):
        ...
        logger.update_stats(
            true=_true,
            pred=_pred,
            batch_size=_true.squeeze().numel(),
...
def train_classification_epoch(model, optimizer, loader, device, scaler, task='classification', use_focal_loss=False):
    ...
    for batch in tqdm(loader, desc="Training", leave=False):
        ...
        """
        有些时候梯度很小，因为精度不够被表示为0
        这就是 GradScaler 出现的背景。它的做法是：放大（scale）损失函数的值，间接使得计算出的梯度更大。这样可以让原本会“下溢”的小梯度在 float16 中还能保留部分有效信息。更新权重前，再将梯度 缩小回来，保持数学意义不变。
        """
        # 收集统计信息 - 与原始代码一致
        logger.update_stats(
            true=_true, 
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.task == 'classification':
            ...
            for test_name, test_loader in test_loaders.items():
                ...
                test_results[test_name] = test_metrics           # 回归指标
                test_class_results[test_name] = test_class_metrics  # 分类指标
