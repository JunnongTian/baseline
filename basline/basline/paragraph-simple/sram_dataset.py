...
class SealSramDataset(InMemoryDataset):
    ...
    def norm_nfeat(self, ntypes):
        ...
        if self._data is None or self.slices is None:
            # 如果 _data_list 为 None，尝试从处理过的文件重新加载
            if self._data_list is None:
                # 对于单数据集情况，直接加载第一个处理过的文件
                self._data, self._slices = torch.load(self.processed_paths[0])
            else:
                # 对于多数据集情况，调用 collate 方法
                self._data, self._slices = self.collate(self._data_list)
            self._data_list = None  # 清理临时列表

        # 确保我们使用的是最新的数据
        data = self._data if self._data is not None else self.data
        
        for ntype in ntypes:
            node_mask = data.node_type == ntype
            max_node_feat, _ = data.node_attr[node_mask].max(dim=0, keepdim=True)
            max_node_feat[max_node_feat == 0.0] = 1.0

            print(f"normalizing node_attr {ntype}: {max_node_feat} ...")
            data.node_attr[node_mask] /= max_node_feat
        
        # 标准化边标签（耦合电容）
        data.edge_label = torch.log10(data.edge_label * 1e21) / 6
        # 屏蔽负边的 edge_label
        data.edge_label[data.edge_label < 0] = 0.0  
